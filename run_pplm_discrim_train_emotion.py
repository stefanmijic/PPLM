...
class Discriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        if pretrained_model.startswith("gpt2") or pretrained_model.endswith("gpt2"):
...
def train_epoch(data_loader, discriminator, optimizer,
    ...
    for batch_idx, (input_t, target_t) in enumerate(data_loader):
        ...
        #weight=torch.tensor([1.0,20.0], device=device)
        #loss = F.nll_loss(output_t, target_t, weight=weight)
...
def train_discriminator(
    ...
    if dataset == "SST":
    ...
    elif dataset == "emotionality":
        x = []
        y = []
        idx2class = ["affect", "cognition"]
        discriminator = Discriminator(
            class_size=len(idx2class),
            pretrained_model=pretrained_model,
            cached_mode=cached,
        ...
        class2idx = {c: i for i, c in enumerate(idx2class)}
        with open("paper_code/emotion_top.csv") as f:
            reader = csv.reader(f)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="SST",
                        choices=("SST", "clickbait", "toxic", "generic", "emotionality"),
